...
# Caminho base
input_dir = "/home/administrador/apptainer-1.3.0/lorenzetti/EVT/output_Upsilon"

# Coletar todos os arquivos ROOT
root_files = glob.glob(f"{input_dir}/job_*/step_5/Upsilon.NTUP.*.root")
...
output_dir = "histogramas_Upsilon"
...
for branch, values in data.items():
    ...
    try:
        ...
        plt.figure()
        plt.hist(numeric_values, bins=20, color='darkred', alpha=0.7)
        plt.title(branch)
        plt.xlabel(branch)
        plt.ylabel("Eventos")
        plt.tight_layout()
