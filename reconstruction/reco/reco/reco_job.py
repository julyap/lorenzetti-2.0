...
class Parallel:
    ...
    def __call__(self, function, **kwargs):
        ...
        # Get function signature and accepted parameters
        sig = inspect.signature(function)
        accepted_params = set(sig.parameters.keys())
        
        # Prepare tasks with strict parameter checking
        tasks = []
        for input_file, output_file, events in jobs:
            ...
            # Filter kwargs to only include accepted parameters not already in base_args
            filtered_kwargs = {
                k: v for k, v in kwargs.items() 
                if k in accepted_params and k not in base_args
            }
            
            # Combine arguments
            final_args = {**base_args, **filtered_kwargs}
            
            # Verify no missing required parameters
            bound_args = sig.bind_partial()
            try:
                bound_args.apply_defaults()
                bound_args.arguments = final_args
            except TypeError as e:
                raise ValueError(f"Parameter mismatch for function {function.__name__}: {str(e)}")
            
            tasks.append(joblib.delayed(function)(**final_args))
